...
/**
 * Servlet implementation class CountryMgrServlet
 */
@WebServlet("/CountryMgrServlet")
public class CountryMgrServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public CountryMgrServlet() {
		super();
	...
	protected void doPost(HttpServletRequest request,
		...
		/*
		 * final String[] memName; final String[] prodName; final String[]
		 * memDesgn;
		 */
		...
		final double opAmt = Double.parseDouble(request.getParameter("opAmt"));
		final double commPer = Double.parseDouble(request
				.getParameter("commPer"));
		final double ope = Double.parseDouble(request.getParameter("ope"));
		final double visitAmt = Double.parseDouble(request
				.getParameter("visitAmt"));
		...
		/*
		 * prodName = request.getParameterValues("prodName"); memName =
		 * request.getParameterValues("memName"); memDesgn =
		 * request.getParameterValues("memDesgn");
		 */
		...
		try {
			...
			String file = "Country Manager - sales.docx";
			...
			docx4j.searchAndReplace(texts, new HashMap<String, Object>() {
				...
				private static final long serialVersionUID = 1L;
				{
					...
					this.put("${ opAmt }", opAmt);
					this.put("${ commPer }", commPer);
					this.put("${ ope }", ope);
					this.put("${ visitAmt }", visitAmt);
					this.put("${ date }", date);
					this.put("${ fodate }", fodate);
					this.put("${ baCountry }", " " + cntry);
					// this.put("${ prodName }", " "+(StringUtils.join(prodName
					// ,", "))+" ");
					this.put("${ tgtAreaName }",
							" " + (StringUtils.join(tgtAreaName, ", ")) + " ");
					// this.put("${ memName }", " "+(StringUtils.join(memName
					// ,", "))+" ");
					// this.put("${ memDesgn }", " "+(StringUtils.join(memDesgn
					// ,", "))+" ");
					this.put("${ pocName }",
			...
			docx4j.writeDocxToStream(
					template,
					"C:\\Users\\BAI-MUM\\Desktop\\Reports\\Country Manager - Sales Reports\\CM-Sales_"
							+ shortName + ".docx");
		} catch (Exception e) {
		...
		RequestDispatcher rd = request.getRequestDispatcher("FileUploadCntryMgrServlet");
